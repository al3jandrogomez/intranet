...
<div id="frmRequisiciones" style="display: none">
	...
	<div class="  form-group row btn-toolbar col-2">
		...
		<button type="button" class="btn btn-warning btn-sm btn-space"
			onclick="reporteExistencia();">Existencia en Almacen</button>
...
<div id="ListaRequisiciones" style="display: block">
	...
	<div class="form-group ">
		<div class="col-2">
			...
			<button type="button" class="btn btn-primary btn-sm btn-space"
				onclick="reporteExistencias();">Reporte de Existencias</button>
		</div>
	</div>
	<div class="form-group ">
		<div class="col-2">
			...
			<button type="button" class="btn btn-primary btn-sm btn-space"
				onclick="reporteExistenciasExcel();">Reporte de Existencias Excel</button>
		</div>
	</div>
	<div class="form-group ">
		<div class="col-2">
			...
			data-whatever="@getbootstrap">Reporte de Distribuci&oacute;n</button>
		</div>
	</div>
...
<div class="modal fade" id="exampleModalRes" tabindex="-1" role="dialog"
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">Consulta de Distribuciones</h5>
				<button type="button" class="close" data-dismiss="modal"
					aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
			...
			<div class="modal-body">
				<form>
					...
					<div class="form-group">
						<label for="message-text" class="col-form-label">Seleccione la Adscripcion</label>
						<select id="cmbListaAds" class="form-control"
			onchange='buscarDistribuciones(this.value)'></select>
					</div>
				</form>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button>
				<button type="button" class="btn btn-primary"
					onclick="buscarDistribuciones()">buscar</button>
			</div>
...
<script>
	...
	buscarDistribuciones = function(cveAdscripcion){
		console.log();
		
	};
	Adscripcion = function(cveAdscripcion){
		this.cveAdscripcion = cveAdscripcion;
	};
	
	cargarAdscripciones = function(){
		var dAdscripcion = new Adscripcion(null);
		dAdscripcion = JSON.stringify(dAdscripcion);
		console.log(dAdscripcion);
		$("#cmbListaAds").empty();
		$
		.ajax({
			data : dAdscripcion,
			url : 'consultaradscripciones',
			dataType : 'json',
			...
			success : function(response) {
				...
				$.each(response,function(index,element){
					combo+="<option value='"+element.cveAdscripcion+"'>"+element.descAdscripcion+"</option>";
				});
				$("#cmbListaAds").append(combo);
	...
	buscarDistribuciones=function(){
		var cveAdscripcion = $("#cmbListaAds").val();
		
		var dAdscripcion = new Adscripcion(cveAdscripcion);
		dAdscripcion = JSON.stringify(dAdscripcion);
		console.log(dAdscripcion);
		...
		.ajax({
			...
			success : function(response) {
				...
				if (response.length > 0) {
									...
									function(index, element) {
										...
										if (element.numeroDistribucion == null) {
											...
											botones = "<img onclick=\"editarDistribucion("
													...
													+ ")\" class='modificar' src=\"resources/images/edit.jpg\"  class=\"img-thumbnail\"><img class=\"eliminar\" onclick=\"eliminarDistribucion("+ element.idDistribucion+")\"	src=\"resources/images/delete.png\"  class=\"img-thumbnail\"/>";
										} else {
											...
											estatus = "Enviada";
											botones += "<img onclick=\"imprimirDocumento2("
												+ element.idDistribucion
					...
					$("#tblListaDistribuciones").append("No se encontraron distribuciones");
				}
			},
			error : function(xhr, ajaxOptions, thrownError) {
				$("#myModalEliminar").modal('hide');
	...
	imprimirDocumento2 = function(idDetalleRequisicion) {
...
// 	reporteExistencias = function(){
		
// 		$
// 		.ajax({
			
// 			url : 'reportecontraloria',
// 			dataType : 'json',
// 			contentType : "application/json",
// 			type : 'post',
// 			beforeSend : function() {
...
// 	};
	...
	reporteExistencias = function() {
		console.log("reporteExistencias");
...
// 		data = idRequisicion;
		;
		var url = "/reportes/reporteexistencias2";
...
// 				key).attr('value', data));
	...
	reporteExistenciasExcel = function() {
		console.log("reporteExistencias");
...
// 		data = idRequisicion;
		;
		var url = "/reportes/reporteexistencias2excel";
